/**
 * realisticVisionGenerator.js
 *
 * Realistic Vision V5 Inpainting integration for generating daily landscape panoramas
 * Uses lucataco/realistic-vision-v5-inpainting model
...
/**
 * Generates a full day landscape by extending an image using Realistic Vision V5 Inpainting
...
export async function generateDailyLandscape(seedImagePath, prompts, outputDir, date) {
    ...
    console.log(`Starting Realistic Vision V5 Inpainting generation for ${date}`);
    console.log(`Using mask-based approach: 854x854 → 1281x854 (scaled for model output)`);
    console.log(`Total segments to generate: ${prompts.length}`);

    const SCALE_FACTOR = 1.67; // Model upscales by ~1.67x
    const BASE_SEED_SIZE = 512;
    const BASE_EXTENSION = 256;

    const SEED_SIZE = Math.round(BASE_SEED_SIZE * SCALE_FACTOR); // 854
    const EXTENSION_WIDTH = Math.round(BASE_EXTENSION * SCALE_FACTOR); // 427
    const CANVAS_WIDTH = SEED_SIZE + EXTENSION_WIDTH; // 1281
    ...
    for (let i = 0; i < prompts.length; i++) {
        ...
        try {
            ...
            // Scale up the seed image to work at model's native resolution
            const scaledSeed = await sharp(imageBuffer)
                .resize(SEED_SIZE, SEED_SIZE, { fit: 'fill' })
                .toBuffer();

            // Create canvas with scaled seed on left, gray on right
            ...
            // Create mask: preserve left SEED_SIZE, paint right EXTENSION_WIDTH
            ...
            // Call Realistic Vision V5 Inpainting
            console.log(`Calling Realistic Vision V5 Inpainting: 768x512 canvas...`);
            let output;
            let retryCount = 0;
            const maxRetries = 3;
            ...
            // Crop rightmost 512px section from output and ensure it's 512x512
            const cropWidth = Math.min(BASE_SEED_SIZE, resultMeta.width);
            const cropHeight = Math.min(BASE_SEED_SIZE, resultMeta.height);
            ...
            await sharp(buffer)
                ...
                .resize(BASE_SEED_SIZE, BASE_SEED_SIZE, { fit: 'fill' })
                .toFile(seedPath);

            currentImagePath = seedPath;
            console.log(`Next seed prepared: ${seedPath} (${BASE_SEED_SIZE}x${BASE_SEED_SIZE})`);
